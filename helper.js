...
const getWindowBadgeText = (windowId) =>
	browser.browserAction.getBadgeText({ windowId: windowId });
...
const setWindowBadgeText = (windowId, text) =>
	browser.browserAction.setBadgeText({ windowId: windowId, text: text });
	...
	new Promise((resolve) => {
		chrome.browserAction.setBadgeBackgroundColor(
			...
			() => {
				if (chrome.runtime.lastError)
					console.error(
						"setTabBadgeBackgroundColor error:",
						chrome.runtime.lastError.message,
					);
				resolve();
			},
	...
	Promise.all([
		new Promise((resolve) => {
			chrome.storage.local.get(null, (localOptions) => {
				if (chrome.runtime.lastError)
					console.error(
						"getStoredOptions error on getting local storage:",
						chrome.runtime.lastError.message,
					);
					...
					chrome.storage.managed.get(null, (managedOptions) => {
						if (chrome.runtime.lastError) {
							if (
								...
								console.warn(
									"could not get managed options:",
									chrome.runtime.lastError.message,
								);
							} else {
								console.error(
									"getStoredOptions error on getting managed storage:",
									chrome.runtime.lastError.message,
								);
							}
						}
