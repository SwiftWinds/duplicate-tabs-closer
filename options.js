...
const initializeOptions = async () => {
	...
	if (storedOptions.length === 0) {
		...
		if (JSON.stringify(storedKeys) != JSON.stringify(defaultKeys)) {
			...
			missingKeys.forEach(
				(key) => (storedOptions[key] = { value: defaultOptions[key].value }),
			);
			const environment = await getEnvironment();
...
const setStoredOption = async (name, value, refresh) => {
	...
	if (refresh) refreshGlobalDuplicateTabsInfo();
		...
		name === "showBadgeIfNoDuplicateTabs" ||
...
const setOptions = (storedOptions) => {
	...
	options.searchInAllWindows =
		storedOptions.scope.value === "A" || storedOptions.scope.value === "CA";
	options.searchPerContainer =
		storedOptions.scope.value === "CC" || storedOptions.scope.value === "CA";
	...
	options.badgeColorNoDuplicateTabs =
		storedOptions.badgeColorNoDuplicateTabs.value;
	options.showBadgeIfNoDuplicateTabs =
		storedOptions.showBadgeIfNoDuplicateTabs.value;
};
